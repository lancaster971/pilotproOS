...
const services = {
  ...
  '4': { key: 'ai', name: 'Automation Engine (n8n)', container: 'pilotpros-n8n' },
...
// Create readline interface - will be created in main()
let rl = null;

// Question function that uses the current rl instance
const question = (query) => {
  return new Promise((resolve) => {
    if (!rl) {
      console.error('Readline not initialized');
      resolve('');
      return;
    }
...
// Main loop
async function main() {
  // Create readline interface here
...
async function start() {
  ...
  // Ensure stdin is in correct mode
