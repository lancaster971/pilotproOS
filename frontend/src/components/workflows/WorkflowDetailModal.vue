          ...
          <div class="space-y-6">
            ...
            <div class="control-card p-4">
              <div class="flex items-center justify-between mb-2">
                <span class="text-text font-medium">Workflow Summary</span>
                <div class="flex items-center">
                  <CheckCircle v-if="workflow.active" class="w-5 h-5 text-primary mr-2" />
                  <XCircle v-else class="w-5 h-5 text-error mr-2" />
                  <span :class="workflow.active ? 'text-primary' : 'text-error'">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p class="text-text font-medium">{{ workflow.node_count }}</p>
                </div>
                <div>
                  <span class="text-text-muted">Executions</span>
                  <p class="text-text font-medium">{{ workflow.execution_count }}</p>
                </div>
                <div>
                  <span class="text-text-muted">Created</span>
                  <p class="text-text font-medium">{{ formatDate(workflow.created_at) }}</p>
                </div>
                <div>
                  <span class="text-text-muted">Updated</span>
                  <p class="text-text font-medium">{{ formatDate(workflow.updated_at) }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="control-card p-6">
              <h3 class="text-lg font-semibold text-text mb-4">Workflow Structure</h3>
              <div class="bg-surface-hover rounded-lg p-4 h-64 flex items-center justify-center border border-border">
                <div class="text-center">
                  <GitBranch class="h-12 w-12 mx-auto mb-2 text-text-muted" />
                  <p class="text-text-muted">Workflow canvas preview</p>
                  <p class="text-xs text-text-muted">{{ workflow.node_count }} nodes connected</p>
                </div>
              </div>
            </div>
            ...
            <div class="control-card p-6">
              ...
              <div class="space-y-2">
                <div v-for="i in 3" :key="i" class="flex items-center justify-between p-3 bg-surface-hover rounded border border-border">
                  <div class="flex items-center gap-2">
                    <CheckCircle class="h-4 w-4 text-primary" />
                    <span class="text-text text-sm">Execution #{{ 1000 + i }}</span>
                  </div>
                  <div class="text-right">
                    <p class="text-xs text-text-muted">{{ formatRelativeTime(Date.now() - i * 3600000) }}</p>
                    <p class="text-xs text-text-muted">Duration: {{ Math.round(Math.random() * 5000) }}ms</p>
                  </div>
                </div>
              </div>
            </div>
