...
class MaintainableRAGSystem:
    ...
    def __init__(
        ...
        try:
            ...
            self.collection = self.chroma_client.create_collection(
                ...
                metadata={"description": "PilotProOS Knowledge Base"}
            )
    ...
    async def create_document(
        ...
        try:
            ...
            embeddings = await self.embeddings_cache.get_embeddings(chunks)
    ...
    async def search(
        ...
        """
        try:
            # Apply masking to query based on user level
            masked_query = self.masking_engine.mask_output(query, user_level)

            # Generate embedding for query
            query_embedding = await self.embeddings_cache.get_embeddings(masked_query)

            # Build metadata filter
            where_filter = filter_metadata or {}
            if not include_archived:
                where_filter["status"] = {"$ne": DocumentStatus.ARCHIVED.value}
                where_filter["status"] = {"$ne": DocumentStatus.DELETED.value}
            ...
            for i, doc_id in enumerate(results["ids"][0]):
                ...
                # Apply masking to content
                masked_content = self.masking_engine.mask_output(content, user_level)
