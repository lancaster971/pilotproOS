...
class MaintainableRAGSystem:
    ...
    async def create_document(
        ...
        try:
            ...
            # CRITICAL FIX (2025-10-17): Run in thread to avoid event loop conflicts
            # ChromaDB's collection.add() is SYNC, but we're in FastAPI async context
    ...
    async def read_document(
        ...
        try:
            ...
            # Query ChromaDB (run in thread to avoid event loop conflicts)
    ...
    async def delete_document(
        ...
        try:
            if hard_delete:
                ...
                # Get IDs to delete (run in thread to avoid event loop conflicts)
                results = await asyncio.to_thread(
                    self.collection.get,
                    where=where_filter,
                    include=[]
    ...
    async def search(
        ...
        try:
            ...
            # Search in ChromaDB (embeddings auto-generated by collection's embedding function)
            # Following official ChromaDB docs: use query_texts for automatic embedding
            # Run in thread to avoid event loop conflicts
            results = await asyncio.to_thread(
                self.collection.query,
                query_texts=[masked_query],  # ChromaDB will embed using embedding function
                n_results=top_k,
            ...
            for i, doc_id in enumerate(results["ids"][0]):
                ...
                # CRITICAL FIX: Skip documents with None/null doc_id (legacy data)
                document_id = metadata.get("doc_id")
                if document_id is None or document_id == "null":
                    logger.warning(f"Skipping search result with invalid doc_id: {document_id}")
                    continue
    ...
    async def list_documents(
        ...
        try:
            ...
            # Query unique documents (run in thread to avoid event loop conflicts)
            results = await asyncio.to_thread(
                self.collection.get,
                where=where_filter if where_filter else None,
                limit=limit,
            ...
            for metadata in results["metadatas"]:
                ...
                if doc_id is None or doc_id == "null":
                    logger.warning(f"Skipping document with invalid doc_id: {doc_id}")
                    continue
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Run in thread to avoid event loop conflicts
            collection_count = await asyncio.to_thread(self.collection.count)
    ...
    async def _archive_version(self, doc_id: str, version: int):
        ...
        try:
            ...
            # Get chunk IDs (run in thread to avoid event loop conflicts)
            results = await asyncio.to_thread(
                self.collection.get,
                where=where_filter,
                include=["metadatas"]
            ...
            for i, chunk_id in enumerate(results["ids"]):
                ...
                # Update in ChromaDB (run in thread to avoid event loop conflicts)
                await asyncio.to_thread(
